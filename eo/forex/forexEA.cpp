 ...
 * class eoforex MUST derive from EO<FitT> for some fitness
 */
#include <eoforex.h>

/** definition of initilizqtion:
 * class eoforexInit MUST derive from eoInit<eoforex>
 */
#include <eoforexInit.h>

/** definition of evaluation:
 * class eoforexEvalFunc MUST derive from eoEvalFunc<eoforex>
 * and should test for validity before doing any computation
 * see tutorial/Templates/evalFunc.tmpl
 */
#include <eoforexEvalFunc.h>
 ...
 */
#include <eoforexQuadCrossover.h>
#include <eoforexMutation.h>

/* and (possibly) your personal statistics */
#include <eoforexStat.h>

// GENOTYPE   eoforex ***MUST*** be templatized over the fitness
...
typedef eoforex<MyFitT> Indi;      // ***MUST*** derive from EO
...
try
  {
    ...
    // The fitness
    //////////////
    eoforexEvalFunc<Indi> plainEval/* (varType  _anyVariable) */;
    // turn that object into an evaluation counter
    eoEvalFuncCounter<Indi> eval(plainEval);

    // a genotype initializer
    eoforexInit<Indi> init;
    // or, if you need some parameters, you might as well
    // - write a constructor of the eoforexInit that uses a parser
    // - call it from here:
    //        eoforexInit<Indi> init(parser);
    ...
    eoforexQuadCrossover<Indi> cross /* (eoParser parser) */;
...
// eoPropCombinedQuadOp<Indi> cross(cross1, cross1Rate);
    ...
    // 1- write the new class by mimicking eoforexQuadCrossover.h
    // 2- include that file here together with eoforexQuadCrossover above
    // 3- uncomment and duplicate the following lines:
    //
// eoforexSecondCrossover<Indi> cross2(eoParser parser);
...
// cross.add(cross2, cross2Rate);
  ...
  eoforexMutation<Indi> mut /* (parser) */;
...
// eoPropCombinedMonOp<Indi> mut(mut1, mut1Rate);
    ...
    // and as many as you want the following way:
    // 1- write the new class by mimicking eoforexMutation.h
    // 2- include that file here together with eoforexMutation above
    // 3- uncomment and duplicate the following lines:
    //
// eoforexSecondMutation<Indi> mut2(eoParser parser);
...
// mut.add(mut2, mut2Rate);
  ...
  eoforexStat<Indi>   myStat;       // or maybe myStat(parser);
  ...
  bool printforexStat = parser.createParam(false, "coutforexStat", "Prints my stat to screen, one line per generation", '\0', "My application").value();
  bool fileforexStat = parser.createParam(false, "fileforexStat", "Saves my stat to file (in resDir", '\0', "My application").value();
  bool plotforexStat = parser.createParam(false, "plotforexStat", "On-line plots my stat using gnuplot", '\0', "My application").value();
  ...
  if (fileforexStat || plotforexStat)
